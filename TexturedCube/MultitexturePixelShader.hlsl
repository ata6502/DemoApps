...
// The pixel shader samples the textures and use them to color pixels.

// Two directional lights
#define LIGHT_COUNT 2

// Multitexture pixel shader accepts two textures.
Texture2D gTextureFlare : register(t0);
Texture2D gTextureAlpha : register(t1);
...
float4 main(PixelShaderInput input) : SV_TARGET
{
    ...
    float4 textureFlareColor = gTextureFlare.Sample(gLinearSampler, input.Tex);
    float4 textureAlphaColor = gTextureAlpha.Sample(gLinearSampler, input.Tex);
    ...
    // Component-wise multiply the corresponding texels of two textures.
    float4 color = textureFlareColor * textureAlphaColor * (ambient + diffuse) + spec;
